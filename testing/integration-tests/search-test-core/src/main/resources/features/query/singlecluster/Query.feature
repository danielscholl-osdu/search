Feature: Search with different queries
  ...
  Scenario Outline: Search data in a given a kind with invalid inputs
    ...
    Examples:
      ...
      | "tenant1" | "tenant1:testquery<timestamp>:well:1.0.0,,tenant1:testquery<timestamp>:well:2.0.0"    | None                        | 1     | None   | 400           | "Bad Request"   | "Invalid parameters were given on search request"                                                | "Not a valid record kind format. Found: tenant1:testquery<timestamp>:well:1.0.0,,tenant1:testquery<timestamp>:well:2.0.0" |
