...
public class SuggestionsQueryUtilTests {
  ...
  public void suggestions_query_not_added_when_featureFlag_enabled() {
    ...
    assertEquals(
        expectedQuery,
        JsonParser.parseString(suggestionsQueryUtil.getSuggestions("aaa").toString()));
  }
  ...
  public void suggestions_are_returned_when_featureFlag_enabled() throws Exception {
    ...
    XContentParser parser =
        JsonXContent.jsonXContent.createParser(
            registry, LoggingDeprecationHandler.INSTANCE, VALID_RESPONSE);
    when(autocompleteFeatureFlag.isFeatureEnabled(AUTOCOMPLETE_FEATURE_NAME)).thenReturn(true);
    ...
    // assertEquals(expectedSuggestions,
    // suggestionsQueryUtil.getPhraseSuggestionsFromSearchResponse(SearchResponse.fromXContent(parser)));
  }
