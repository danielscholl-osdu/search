...
**hpa.targetValue** | threshold value to trigger the scaling up | integer | `30` | only if `global.autoscaling` is true and `global.tier` is PROD
...
| Name | Description | Type | Default |Required |
|------|-------------|------|---------|---------|
**limits.maxTokens** | maximum number of requests per fillInterval | integer | `20` | only if `global.autoscaling` is true and `global.tier` is PROD
**limits.tokensPerFill** | number of new tokens allowed every fillInterval | integer | `20` | only if `global.autoscaling` is true and `global.tier` is PROD
